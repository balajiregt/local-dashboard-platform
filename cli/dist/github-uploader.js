...
class GitHubUploader {
    ...
    async uploadReport(report, assetPaths = []) {
        ...
        try {
            ...
            this.spinner.succeed(`Report uploaded successfully! View at: ${chalk_1.default.blue(result.dashboardUrl)}`);
            return result;
        }
        catch (error) {
    ...
    async ensureReportsStructure() {
        ...
        try {
            ...
            if (error.response?.status === 404) {
                ...
                await this.createOrUpdateFile('reports/index.json', JSON.stringify(initialIndex, null, 2), 'Initialize reports index');
            }
        }
    }
    async ensureDirectory(path) {
        try {
            ...
            if (error.response?.status === 404) {
                ...
                await this.createOrUpdateFile(`${path}/.gitkeep`, '', `Create ${path} directory`);
            }
        }
    }
    async uploadReportData(reportPath, report) {
        const reportJson = JSON.stringify(report, null, 2);
        await this.createOrUpdateFile(`${reportPath}/report.json`, reportJson, `Add test report ${report.execution.reportNumber}`);
        // Create a summary markdown file
        const summaryMd = this.generateReportSummary(report);
        await this.createOrUpdateFile(`${reportPath}/summary.md`, summaryMd, `Add report summary ${report.execution.reportNumber}`);
    }
    async uploadAssets(reportPath, assetPaths) {
        ...
        for (const assetPath of assetPaths) {
            try {
                ...
                await this.createOrUpdateFile(targetPath, content, `Add test asset ${fileName}`, true // binary content
                );
    ...
    async prepareAssetForUpload(filePath) {
        const stats = (0, fs_1.statSync)(filePath);
        // For large files, compress them
        if (stats.size > 1024 * 1024) { // 1MB
            if (filePath.endsWith('.zip')) {
                // Already compressed
                return (0, fs_1.readFileSync)(filePath, 'base64');
            }
            else {
                // Compress the file
    ...
    async updateReportsIndex(report) {
        try {
            ...
            await this.createOrUpdateFile('reports/index.json', JSON.stringify(index, null, 2), `Update reports index with report ${report.execution.reportNumber}`, false, response.data.sha);
        }
        catch (error) {
            console.warn('Failed to update reports index:', error);
        }
    }
    async createOrUpdateFile(path, content, message, isBinary = false, sha) {
        const payload = {
            message,
        ...
        await this.client.put(`/repos/${this.config.owner}/${this.config.repo}/contents/${path}`, payload);
    }
